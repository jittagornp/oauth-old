 ...
 * {@link SessionDeletedEvent} and {@link SessionExpiredEvent} by implementing
 ...
 * <pre>
 ...
 RedisUserSessionRepository redisSessionRepository = new RedisUserSessionRepository(factory);
 </pre>
 *
 * <p>
 ...
 * {@link RedisUserSessionRepository} will then translate the Redis
 ...
 * @since 1.0
 */
public class RedisUserSessionRepository implements FindByIndexNameSessionRepository<RedisUserSessionRepository.RedisSession>, MessageListener {
    ...
    /**
     * The default prefix for each key and channel in Redis used by Spring
     * Session.
     */
    ...
    /**
     * The prefix of the key for used for session attributes. The suffix is the
     * name of the session attribute. For example, if the session contained an
     * attribute named attributeName, then there would be an entry in the hash
    ...
    /**
     * Allows creating an instance and uses a default {@link RedisOperations}
     * for both managing the session and the expirations.
     *
     ...
     *
     * @param defaultMaxInactiveInterval the number of seconds that the
     * {@link Session} should be kept alive between client requests.
     */
    ...
    /**
     * Sets the default redis serializer. Replaces default serializer which is
     * based on {@link JdkSerializationRedisSerializer}.
     *
    ...
    /**
     * Sets the redis flush mode. Default flush mode is
    ...
    private void cleanupPrincipalIndex(RedisSession session) {
        LOG.debug("cleanupPrincipalIndex({})...", session);
        if (session == null) {
            return;
        }
    ...
    /**
     * Gets the prefix for the channel that SessionCreatedEvent are published
     * to. The suffix is the session id of the session that was created.
     *
     * @return the prefix for the channel that SessionCreatedEvent are published
     * to
     */
     ...
     * as the basis for its mapping. It keeps track of any attributes that have
     * changed. When
    ...
    final class RedisSession implements ExpiringSession {
         ...
         *
         * @param cached the {@link MapSession} that represents the persisted
         * session that was retrieved. Cannot be null.
         */
